...
def connect_one(ip: str, port: int, timeout: float) -> tuple[Optional[socket.socket], Optional[float], Optional[str]]:
    """
...
def connect_with_fallback(ips: list[str], port: int, timeout: float, prefer: str = "any") -> TCPConnectResult:
    ...
    ## prefer 정책 any: DNS 결과 순서 그대로 시도
    ## ipv4: IPv4 우선시도, ipv6: IPv6 우선시도
    
    ordered = []

    v4 = [ip for ip in ips if ':' not in ips]
    v6 = [ip for ip in ips if '.' not in ips]
    ...
    for ip in ordered:
        ...
        sock, connect_ms, error_ms = connect_one(ip=ip, port=port, timeout=timeout)
