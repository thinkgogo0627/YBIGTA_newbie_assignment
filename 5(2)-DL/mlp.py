...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h2 = np.matmul(z1, W2) + b2
        z2 = leakyrelu(h2)

        h3 = np.matmul(z2, W3) + b3
        z3 = tanh(h3 + h1)

        h4 = np.matmul(z3, W4) + b4

        # numpy 내부에는 sigmoid가 없음 -> 직접 구현
        y_hat = sigmoid(h4)

        # np.reshape(배열, 모양)
        y_hat = np.reshape(y_hat, X.shape[0])
        #################
    ...
    def train(
        ...
        for it in range(epoch):
            ...
            for start in range(0, n, effective_batch_size):
                ...
                y_hat, cache = self.forward_propagation(X=X_batch)
                loss = self.compute_loss(y_pred=y_hat, y_true=y_batch, L2_norm=L2_norm)
                grad = self.back_propagation(cache=cache, X=X_batch, y=y_batch, L2_norm=L2_norm)


                ## back_propagation(self, cache, X, y, L2_norm=0.0):
                # Gradient update
